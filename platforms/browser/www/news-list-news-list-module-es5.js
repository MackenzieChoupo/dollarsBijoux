    ...
    var NewsListPage = /*#__PURE__*/function () {
      ...
      _createClass(NewsListPage, [{
        ...
        value: function getPosts() {
          ...
          this.config.getWithUrl(this.config.url + '/wp-json/wp/v2/posts/?page=' + this.page + "&categories=" + this.id + "&" + this.config.productsArguments).then(function (data) {
            ...
            if (data.length < 9) {
              ...
              _this.infinite.disabled = true; //disabling infinite scroll

              if (_this.posts.length != 0) {// this.shared.toast("All Posts Loaded!");
              }
            }

            _this.loadingServerData = true;
          }, function (err) {
            _this.infinite.disabled = true; // console.log("Error while loading posts from the server");
